...
export async function sendPasswordResetEmail(to: string, resetUrl: string): Promise<void> {
  ...
  const { data, error } = await resend.emails.send({
    ...
    to,
    subject: 'Reset your Pattern Deck password',
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
        <h2 style="color: #1a1a1a; margin-bottom: 16px;">Reset your password</h2>
        <p style="color: #555; line-height: 1.6; margin-bottom: 24px;">
          You requested a password reset for your Pattern Deck account. Click the button below to choose a new password. This link expires in 5 minutes.
        </p>
