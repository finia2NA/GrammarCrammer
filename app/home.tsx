...
import { generateDeckExplanation } from '@/lib/explanation-generator';
    ...
    try {
      if (editNode) {
        ...
        if (editNode.deck !== null) {
          ...
          const { regenerateExplanation } = await updateDeck(editNode.id, {
            name: pathChanged ? undefined : newName,
            topic: data.topic,
            language: data.language,
            cardCount: data.cardCount,
          });
          if (regenerateExplanation) {
            generateDeckExplanation(editNode.id);
          }
        }
      } else {
        const deckId = await createDeckFromPath(data.path, data.topic, data.language, data.cardCount);
        generateDeckExplanation(deckId);
      }
